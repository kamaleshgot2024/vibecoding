...
@app.command()
def connect_to_cluster(namespace: str = "kube-lab"):
    """
    Connect to a Minikube cluster and use the specified namespace.
    """
    try:
        # Load Minikube's kubeconfig
        from kubernetes import config, client

        config.load_kube_config(context="minikube")
        typer.echo("Connected to Minikube cluster.")

        # Check if the namespace exists
        v1 = client.CoreV1Api()
        namespaces = [ns.metadata.name for ns in v1.list_namespace().items]
        if namespace not in namespaces:
            typer.echo(f"Namespace '{namespace}' not found. Creating it...")
            ...
            typer.echo(f"Using existing namespace: '{namespace}'.")

        # Set the namespace for further operations
        typer.echo(f"Namespace set to: {namespace}")
...
def diagnose(
    ...
    namespace: str = typer.Option("kube-lab", "--namespace", "-n", help="Kubernetes namespace"),
...
def scan(
    namespace: str = typer.Option("kube-lab", "--namespace", "-n", help="Kubernetes namespace to scan"),
...
def logs(
    ...
    try:
        ...
        if follow:
            ...
            if pod_name is None:
                console.print("[red]❌ Pod name cannot be None[/red]")
                raise typer.Exit(1)
...
#from your_correct_module_name import app  # Replace 'your_correct_module_name' with the actual module name
